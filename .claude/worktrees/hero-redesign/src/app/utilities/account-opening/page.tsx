...
const INPUT_CLASS =
  "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#112330] placeholder:text-slate-400 outline-none transition-all duration-150 focus:ring-2 focus:ring-[#F2A123] focus:border-transparent";

const LABEL_CLASS = "text-[0.7rem] font-bold tracking-[0.12em] uppercase text-[#112330]/50";
...
export default function AccountOpeningPage() {
      ...
      return (
        <fieldset className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-6">
            {field.options?.map((option) => (
              <label key={option} className="inline-flex items-center gap-2 text-sm font-semibold text-[#112330]">
                <input
                  type="radio"
                  name={field.name}
                  value={option}
                  required={field.required}
                  className="h-4 w-4 accent-[#F2A123]"
                />
                {option}
              </label>
            ))}
    ...
    if (field.type === "file") {
      return (
        <div className="space-y-2">
          ...
          <label
            htmlFor={field.name}
            className="group relative flex min-h-[120px] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center transition-colors hover:border-[#F2A123]/70 hover:bg-[#fffaf0]"
          >
            <svg className="mb-3 h-6 w-6 text-slate-400 group-hover:text-[#F2A123]" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            ...
            <span className="text-sm font-semibold text-[#112330]">Drag & drop file here or click to upload</span>
  ...
  return (
    <main className="bg-[#F8F9FA] text-[#112330] font-sans">
      <section className="bg-white border-b border-slate-100">
        <div className="max-w-4xl mx-auto py-12 px-6">
          <h1 className="text-4xl font-black text-[#112330] tracking-[-0.03em]">{pageTitle}</h1>
        ...
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSubmit} className="space-y-8">
            {sections.map((section) => (
              ...
              >
                <h2 className="text-2xl font-bold text-[#112330] tracking-[-0.02em] mb-2">{section.title}</h2>
                {section.description ? (
            ...
            <div className="bg-white shadow-sm border border-slate-200 rounded-xl p-8">
              <label className="flex items-start gap-3 text-sm text-slate-700 leading-relaxed">
                <input
                  ...
                  className="mt-0.5 h-4 w-4 rounded accent-[#F2A123]"
                  required
                />
                <span>
              ...
              <button
                type="submit"
                className="mt-6 w-full inline-flex items-center justify-center rounded-xl bg-[#112330] px-6 py-4 text-white font-bold tracking-wide transition-colors duration-200 hover:bg-[#0b1a24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F2A123]"
              >
