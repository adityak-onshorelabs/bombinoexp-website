...
function ServiceRow({
  ...
  return (
    <motion.a
      href={service.href}
      className="grid grid-cols-12 gap-6 items-start border-b border-slate-200 py-12 w-full group cursor-pointer"
      initial={{ opacity: 0, y: 18 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={VIEWPORT}
      transition={{ duration: 0.6, ease: PREMIUM_EASE, delay: index * 0.04 }}
    >
      {/* Col 1-2: Amber number */}
      <div className="col-span-2 pt-1">
        <span className="text-2xl font-bold text-[#F2A123]">{service.number}</span>
      </div>

      {/* Col 3-9: Title + description */}
      <div className="col-span-7 flex flex-col gap-4 w-full">
        <h3 className="text-4xl font-semibold text-[#112330] tracking-tight leading-tight group-hover:text-[#1E567B] transition-colors duration-300">
          {service.title}
        </h3>
        <p className="text-slate-500 leading-relaxed w-full">
          {service.desc}
        </p>
      </div>

      {/* Col 10-12: Massive thin-stroke icon */}
      <div className="col-span-3 flex justify-end items-start">
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={VIEWPORT}
          transition={{ duration: 0.7, ease: PREMIUM_EASE, delay: index * 0.04 + 0.15 }}
        >
          <Icon
            size={128}
            strokeWidth={1}
...
export function ServicesEditorial() {
  return (
    <section
      ...
      className="bg-white"
    >
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start relative py-32">
        ...
        <div className="lg:col-span-5 sticky top-32 h-fit">
          ...
          >
            ...
            <h2
              id="services-heading"
              className="font-extrabold text-admiralty leading-[1.03] tracking-[-0.03em]"
            ...
            <p className="text-muted-ink text-lg leading-relaxed">
              From a single parcel to a full freight container — we cover every shipping need with
              speed, precision, and 30 years of expertise.
            </p>
