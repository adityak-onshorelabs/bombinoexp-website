...
export function ServicesOverview() {
  return (
    <section className="py-24 bg-white">
      <div className="w-full max-w-7xl mx-auto px-4 md:px-6 space-y-16">
        <div className="text-center max-w-2xl mx-auto space-y-4">
          <h2 className="text-4xl md:text-5xl font-bold text-[#112330]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map(({ icon: Icon, title, desc, href }) => (
            <div key={title} className="bg-white border border-slate-200 rounded-2xl p-8 flex flex-col">
              ...
              <h3 className="text-xl font-bold text-[#112330] mt-6 mb-3">{title}</h3>
              <p className="text-slate-600 leading-relaxed flex-1">{desc}</p>
              <a href={href} className="text-[#F2A123] font-semibold hover:underline mt-6 inline-block">
                Explore Service →
              </a>
            </div>
          ))}
