...
function OfficeCard({ office, index, delay }: { office: (typeof offices)[number]; index: number; delay: number }) {
  return (
    <motion.div
      className="min-h-[320px] md:min-h-[420px] bg-white rounded-[2rem] p-6 md:p-10 flex flex-col justify-between relative overflow-hidden group shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 hover:border-[#FBAD1F]/30 transition-all duration-500"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center">
        ...
        <ul className="space-y-3">
          ...
          <li className="flex gap-3 items-center">
            ...
            <a href={`tel:${office.phone.replace(/\s/g, "")}`} className="text-slate-500 text-sm hover:text-[#FBAD1F] transition-colors duration-150">
              {office.phone}
            </a>
          </li>
          <li className="flex gap-3 items-center">
            <Mail className="w-4 h-4 text-slate-400 shrink-0" aria-hidden="true" />
            <a href={`mailto:${office.email}`} className="text-slate-500 text-sm hover:text-[#FBAD1F] transition-colors duration-150 truncate">
              {office.email}
            </a>
          </li>
        </ul>
    ...
    >
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
        ...
        <FadeUp className="mb-12">
          ...
          <h2 id="offices-heading" className="text-headline text-[lab(7.78673_1.82346_-15.0537)]">
            Global head offices.
          </h2>
        </FadeUp>
