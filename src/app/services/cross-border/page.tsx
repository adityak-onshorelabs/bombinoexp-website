...
const howItWorksIcons = [
  <svg key="truck" className="w-6 h-6 text-[#FBAD1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  <svg key="shield" className="w-6 h-6 text-[#FBAD1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 3l7 4v5c0 5-3.5 8-7 9-3.5-1-7-4-7-9V7l7-4z" />
  </svg>,
  <svg key="box" className="w-6 h-6 text-[#FBAD1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
  </svg>,
  <svg key="network" className="w-6 h-6 text-[#FBAD1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h8M12 8v8M5 5h.01M19 5h.01M5 19h.01M19 19h.01M5 5l14 14M19 5L5 19" />
  </svg>,
];

export default function CrossBorderPage() {
  const { hero, howItWorks, globalShipping, whyChooseUs } = crossBorderData;
    ...
    <main className="font-sans text-foreground">
      ...
      <section className="bg-[#F8FAFC] py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="mb-14">
            <p className="text-[0.65rem] font-bold tracking-[0.2em] uppercase text-[#FBAD1F] mb-3">Process</p>
            <h2 className="text-4xl lg:text-5xl font-bold text-foreground leading-tight tracking-[-0.03em] mb-4">
              {howItWorks.title}
            </h2>
            <p className="text-slate-500 text-lg">{howItWorks.description}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {howItWorks.items.map((item, i) => (
              <div
                key={i}
                className="group relative bg-white rounded-[2rem] p-10 lg:p-12 overflow-hidden border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:border-[#FBAD1F]/30 transition-all duration-500 flex flex-col h-full"
              >
                ...
                <div className="relative z-10 flex-1 flex flex-col">
                  <div className="w-14 h-14 rounded-xl bg-[#FBAD1F]/10 flex items-center justify-center mb-8 shrink-0">
                    {howItWorksIcons[i % howItWorksIcons.length]}
                  </div>
      ...
      <section className="bg-[#F8FAFC] py-16 lg:py-24 border-t border-slate-100 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="max-w-4xl">
            <h2 className="text-4xl lg:text-5xl font-bold text-foreground leading-tight tracking-[-0.03em] mb-8">
              {globalShipping.title}
            </h2>
            <div className="space-y-5 text-slate-600 text-base lg:text-lg leading-relaxed text-left">
              {globalShipping.paragraphs.map((paragraph, idx) => (
                <p key={idx}>{paragraph}</p>
              ))}
            </div>
          </div>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="mb-14">
            <p className="text-[0.65rem] font-bold tracking-[0.2em] uppercase text-[#FBAD1F] mb-3">Benefits</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {whyChooseUs.items.map((item, i) => (
              <div
                key={i}
                className="flex items-start gap-4 bg-[#F8FAFC] rounded-2xl p-6 border border-slate-100 hover:border-[#FBAD1F]/30 transition-colors duration-300"
              >
                <div className="shrink-0 w-8 h-8 rounded-lg bg-[#FBAD1F]/10 flex items-center justify-center mt-0.5">
                  <svg className="w-4.5 h-4.5 text-[#FBAD1F]" viewBox="0 0 18 18" fill="none" aria-hidden="true">
