	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			const res = await fetch("/api/sheets", {
				...
				body: JSON.stringify({ type: "customs", ...formData }),
			});
		...
		<main className="font-sans text-foreground">
			...
			<section className="relative w-full overflow-hidden">
				<div className="absolute inset-0 z-0">
					...
					/>
					<div className="absolute inset-0 bg-admiralty/85" />
				</div>
					...
					<div className="w-full lg:w-[448px] shrink-0">
						<div className="bg-white text-slate-800 rounded-xl p-6 lg:p-8 shadow-2xl border border-slate-100">
							...
							<form onSubmit={handleSubmit} className="space-y-4">
									...
									<div key={field.name}>
										...
										{field.type === "select" ? (
											<select
												...
												className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-800 outline-none focus:ring-2 focus:ring-[#FBAD1F] focus:border-transparent transition">
											...
											<input
												...
												value={formData[field.name] ?? ""}
												onChange={handleChange}
												className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#FBAD1F] focus:border-transparent transition"
											/>
										)}
									</div>
								))}
								...
								{status === "success" && (
									<p className="text-sm font-semibold text-green-600 bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-center">
										Request submitted! We'll be in touch shortly.
									</p>
								)}
								{status === "error" && (
									<p className="text-sm font-semibold text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-center">
								...
								<button
									...
									className="w-full mt-2 bg-admiralty text-white text-sm font-bold py-3 rounded-lg hover:bg-[oklch(0.38 0.08 240)] active:scale-[0.98] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
									{status === "loading" ? "Submitting…" : heroForm.buttonText}
								</button>
							</form>
						</div>
					</div>
