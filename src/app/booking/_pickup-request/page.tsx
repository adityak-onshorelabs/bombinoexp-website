...
const badgeClass =
  "bg-slate-50 text-foreground px-3 py-1.5 rounded mb-6 text-sm font-extrabold tracking-wider inline-block uppercase";
      ...
      <main className="font-sans text-foreground bg-[#F8FAFC] min-h-screen pb-20">
        <section className="bg-white border-b border-slate-100 py-16 lg:py-20">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <p className="text-[0.65rem] font-bold tracking-[0.2em] uppercase text-[#FBAD1F] mb-4">Door-to-Door Dispatch</p>
            <h1 className="text-4xl lg:text-5xl font-black text-foreground tracking-tight leading-tight mb-4">{header.title}</h1>
            ...
            <div className="m-6 sm:m-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className="bg-[#F8FAFC] rounded-xl p-5 border border-slate-100">
                  <p className="text-[0.65rem] font-bold tracking-[0.18em] uppercase text-[#FBAD1F] mb-1">Reference Number</p>
                  <p className="text-2xl font-black text-foreground tracking-tight">{refNumber}</p>
                  <p className="text-xs text-slate-500 mt-1">Save this for tracking</p>
                </div>
                <div className="bg-[#F8FAFC] rounded-xl p-5 border border-slate-100">
                  <p className="text-[0.65rem] font-bold tracking-[0.18em] uppercase text-[#FBAD1F] mb-1">Pickup Date</p>
                  <p className="text-2xl font-black text-foreground tracking-tight">{formData.pickupDate || "—"}</p>
              ...
              <div className="bg-admiralty/4 rounded-xl p-4 text-sm text-slate-600 leading-relaxed mb-6">
                A confirmation will be sent to{" "}
                <span className="font-semibold text-foreground">{formData.pickupEmail || "your email"}</span>. Our team will reach out if any details need verification.
              </div>

              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={reset} className="flex-1 bg-admiralty text-white font-bold py-3 rounded-lg hover:bg-[oklch(0.38 0.08 240)] transition-colors duration-200">
                  Schedule Another Pickup
                </button>
                <a href="/contact" className="flex-1 text-center border border-slate-200 text-foreground font-semibold py-3 rounded-lg hover:border-[#FBAD1F] hover:text-[#FBAD1F] transition-colors duration-200">
                  Contact Support
                </a>
              </div>
            </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-6 mt-12 space-y-6">
        ...
        <div className="flex items-center justify-center py-2">
          <div className="flex flex-col items-center gap-1 text-slate-400">
            ...
            <svg className="w-6 h-6 text-[#FBAD1F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-[#FBAD1F] focus:border-transparent transition resize-none"
              />
            </div>
          </div>
